...
if($res==0){
	...
	header("Location:Contact.php?error=emptyfields&name=".$name."&email=".$email."&subject=".$subject."&message=".$message);
	exit();
       }
       else if(!filter_var($email,FILTER_VALIDATE_EMAIL)){
	header("Location:Contact.php?error=invalidemail&name=".$name);
	exit();
      }
      else{
      	$sql = "INSERT INTO persons(Name,Email,Subject,Message) VALUES ('$name','$email','$subject','$message')";
	...
	header("Location:Contact.php?error=contactunsuccess".$Email);
}
